...
template <typename T> struct Point3d {
  ...
  T angle(const Point3d &p) const {
    return std::acos(dot(p) / (abs() * p.abs()));
  }
  T angle(const Point3d &p1, const Point3d &p2) const {
    return (p1 - *this).angle(p2 - *this);
  }
  T length() const { return abs(); }

  Point3d rotateX(T rad) const {
    return Point3d(x, y * std::cos(rad) - z * std::sin(rad),
                   y * std::sin(rad) + z * std::cos(rad));
  }
  Point3d rotateY(T rad) const {
    return Point3d(x * std::cos(rad) + z * std::sin(rad), y,
                   -x * std::sin(rad) + z * std::cos(rad));
  }
  Point3d rotateZ(T rad) const {
    return Point3d(x * std::cos(rad) - y * std::sin(rad),
                   x * std::sin(rad) + y * std::cos(rad), z);
  }
  Point3d rotate(const Point3d &axis, T rad) const {
    return (*this - axis).rotateZ(rad) + axis;
  }
  Point3d rotate(const Point3d &axis, const Point3d &dir, T rad) const {
    return (*this - axis).rotate(dir, rad) + axis;
  }
