...
#include <FleursLibrary/geometrythd/linethd.hpp>
#include <FleursLibrary/geometrythd/pointthd.hpp>

namespace fleurs {

template <typename T> struct segment3d {
  Point3d<T> a, b;
  ...
  segment3d<T> &operator=(const segment3d<T> &l) {
    a = l.a;
    b = l.b;
    return *this;
  }
  bool operator==(const segment3d<T> &l) const {
  ...
  bool isOrthogonal(const segment3d<T> &l) const {
    return equals(dot(*this, l), 0.0);
  }
  bool isParallel(const segment3d<T> &l) const {
    return equals(cross(*this, l), 0.0);
  }
  bool intersect(const segment3d<T> &l) const {
    if (std::max(a.x, b.x) < std::min(l.a.x, l.b.x) ||
        ...
        std::max(l.a.z, l.b.z) < std::min(a.z, b.z))
      return false;
  }
  ...
  Point3d<T> crosspoint(const Line3d<T> &l) const {
    T A = cross(b - a, l.b - l.a);
    T B = cross(b - a, b - l.a);
    if (equals(A, 0) && equals(B, 0))
      return l.a;
