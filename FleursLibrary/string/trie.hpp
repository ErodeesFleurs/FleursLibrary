...
struct binary_trie{
	int n = 0;
	std::vector<std::array<int, 3>> next{{0, 0, 0}}; // leftchild, rightchild, count
	binary_trie(){ }
...
struct trie{
	...
	trie(){ }
	trie(const std::vector<Adjacency_Type> &next, const std::vector<int> &cnt): n((int)next.size()), next(next), cnt(cnt){ }
	int extend(){
		next.emplace_back();
		exist.emplace_back();
		cnt.push_back(0);
