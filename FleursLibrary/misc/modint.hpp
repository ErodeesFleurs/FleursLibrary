...
struct modint {
    ...
    modint& operator+=(const modint& m) { if ((x += m.x) >= MOD) x -= MOD; return *this; }
    modint& operator-=(const modint& m) { if ((x += MOD - m.x) >= MOD) x -= MOD; return *this; }
    modint& operator*=(const modint& m) { x = (long long)x * m.x % MOD; return *this; }
