...
template <typename T> struct Point {
  ...
  Point operator-(void) const { return Point<T>(-x, -y); }
  ...
  Point operator*(T s) const { return Point<T>(x * s, y * s); }
  ...
  T angle(const Point<T> &p) const { return std::atan2(cross(p), dot(p)); }
  T angle(const Point<T> &a, const Point<T> &b) const {
    return (a - *this).angle(b - *this);
  }
  T distance(const Point<T> &p) const { return (p - *this).abs(); }
  ...
  Point<T> rotate90() const { return Point<T>(-y, x); }
  Point<T> normalize() const { return *this * (1.0 / abs()); }
  Point<T> normalize(const Point<T> &p) const {
    return (*this - p).normalize() + p;
  }
  Point<T> normal() const { return rotate90().normalize(); }
